...
The video asks the pre-tool chatbot three things: the latest news, Apple's current stock price, and
...
The video breaks a single tool-using turn into eight steps, and the graph mirrors them:
...
tool, so write it for the model. The video builds three: `calculator` (evaluates a math
...
tool (the video chains `get_stock_price` and then `calculator` to price 50 shares in one question).
...
The chat node, state, checkpointer and Streamlit app are otherwise unchanged. In the UI the video
...
def calculator(expression: str) -> str:
    ...
    try:  # sketch: the video evaluates the expression with the math module available
...
- **Using `llm` instead of `llm_with_tools` in the chat node.** The graph compiles, the model never emits a tool call, and it looks like "tools don't work". The video calls this the most common mistake.
- **Tool defined but not in the list.** The weather tool failed in the video because it was never added to `tools` before `bind_tools`. `ToolNode` and the model only know what is in that list.
...
- **Provider swap.** The video switched from OpenAI to Gemini mid-series when a quota ran out. Only the model constructor changes; `bind_tools`, `ToolNode` and `tools_condition` are model-agnostic.
...
<div class="yt">
  <iframe
    src="https://www.youtube-nocookie.com/embed/pkwVn8ZtRvE"
    title="19. How to Integrate Tools in Agentic Chatbot with LangGraph / Part 7"
    loading="lazy"
    allowfullscreen
  ></iframe>
</div>

Source: DSwithBappy, [19. How to Integrate Tools in Agentic Chatbot with LangGraph / Part 7](https://www.youtube.com/watch?v=pkwVn8ZtRvE).
